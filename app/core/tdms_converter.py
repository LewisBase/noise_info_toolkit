...
class TDMSConverter:
    ...
    def convert_tdms_to_wav(self, tdms_file_path: str, wav_file_path: str = None) -> str:
        ...
        """
        try:
            # If no output path specified, create one based on the input file
            if wav_file_path is None:
                tdms_path = Path(tdms_file_path)
                wav_file_path = tdms_path.with_suffix('.wav')
            ...
            sf.write(wav_file_path, data, sampling_rate)
    ...
    def batch_convert_tdms_files(self, tdms_directory: str, wav_directory: str = None) -> list:
        ...
        for tdms_file in tdms_files:
            try:
                wav_file_path = Path(wav_directory) / f"{tdms_file.stem}.wav"
